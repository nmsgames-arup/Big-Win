...
namespace WOF.Gameplay
{
    ...
    public class WOF_Timer : MonoBehaviour
    {
        public static WOF_Timer Instance;
        int bettingTime = 15;
        int timeUpTimer = 10;
        int waitTimer = 3;
        ...
        void Start()
        {
            ...
            if(is_a_FirstRound)
            {
                WOF_UiHandler.Instance.ShowMessage("please wait for next round...");
            }
        }
        ...
        public void OnTimerStart(object data)
        {
            if (is_a_FirstRound)
            {
                WOF_UiHandler.Instance.HideMessage();
            }
        ...
        {
            WOF_UiHandler.Instance.HideMessage();
            WOF_UiHandler.Instance.DragonAnimpanel.gameObject.SetActive(true);
            UtilitySound.Instance.VSsoundbtn();
            foreach (var item in WOF_UiHandler.Instance.DragonAnimFrames)
            {
                WOF_UiHandler.Instance.DragonAnimpanel.sprite = item;
                yield return new WaitForSeconds(0.045f);
            }
            WOF_UiHandler.Instance.DragonAnimpanel.gameObject.SetActive(false);
        }
        ...
        public void OnCurrentTime(object data = null)
        {
            ...
            try
            {
                ...
                WOF_UiHandler.Instance.UpDateBalance(float.Parse(cr.balance));
            }
            catch (Exception e)
            {
                Debug.Log(e.Message);
